...
SearchQueue.prototype = {
   _process: function(snap) {
      ...
      if( this._assertValidSearch(snap.name(), snap.val()) ) {
         var term = makeTerm(dat, this.matchWholeWords);
   ...
   _nextInterval: function() {
      ...
      console.log('Next cleanup in %d %s'.grey, Math.round(this.cleanupInterval/(interval==='seconds'? 1000 : 60000)), interval); //debug
