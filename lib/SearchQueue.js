...
SearchQueue.prototype = {
  _process: function(snap) {
     ...
     if (this._assertValidSearch(key, dat)) {
       ...
       if( dat.hasOwnProperty('query') ) {
         // we look for dat.query for backward compatibility
         if( typeof dat.query === 'string' ) { searchProps.q = dat.query ; }
         else { searchProps.body = dat.query ; }
       }
       else if( dat.hasOwnProperty('q') ) {
         searchProps.q = dat.q;
       }
       else {
         if( typeof dat.body === 'string' ) {
           dat.body = JSON.parse(dat.body);
         }
         searchProps.body = dat.body;
       }
